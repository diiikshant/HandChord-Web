...
export function PersonalSounds({ engine, onBeforeSourceChange }: Props) {
  ...
  return <section className="reverb-panel" aria-labelledby="personal-sounds-title">
    <div className="gesture-audio-heading"><div><p className="eyebrow">Local-only audio</p><h2 id="personal-sounds-title">Personal Sounds</h2></div><label className="primary-button">Add Personal Sound<input type="file" accept="audio/*,.wav,.mp3,.m4a,.aac,.ogg" hidden onChange={(event) => { void importFile(event.target.files?.[0]); event.currentTarget.value = '' }} /></label></div>
    <p className="gesture-audio-message">Import up to 10 seconds or 12 MB. Audio stays in this browser’s IndexedDB and is never uploaded.</p>
